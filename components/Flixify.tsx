...
const Flixify: FC = () => {
    ...
    const { data } = useQuery(GET_ALL_MOVIES,
        {
            onCompleted: (data) => {
                const latestMovies: IMovie[] = data.popularMovies.map((movie: MovieFromQuery) => processMovie(movie));
                setLatestReleases(latestMovies);
                const nowPlayingMovies: IMovie[] = data.nowPlayingMovies.map((movie: MovieFromQuery) => processMovie(movie));;
